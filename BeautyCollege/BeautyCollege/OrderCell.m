...
//        [img sd_setImageWithURL:[NSURL URLWithString:model.logo]];
        ...
        label1 = [[UILabel alloc] initWithFrame:CGRectMake(img.right + 5, 10, frame.size.width - img.right - 10, 40)];
...
//        Label2.text = model.name;
        ...
        label3 = [[UILabel alloc] initWithFrame:CGRectMake(Label2.left, Label2.bottom, Label2.width / 2.0, 20)];
...
- (instancetype)initWithFrame:(CGRect)frame withModel:(BaseCellModel *)model
{
    ...
    if (self) {
        ...
        label1 = [[UILabel alloc] initWithFrame:CGRectMake(img.right + 5, img.top, frame.size.width - img.right - 10, 40)];
        label1.numberOfLines = 2;
        label1.font = [UIFont systemFontOfSize:15];
        label1.text = model.title;
        [self addSubview:label1];
